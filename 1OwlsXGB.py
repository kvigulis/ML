...
for i in range(fold):
    ...
    x1, x2, y1, y2 = model_selection.train_test_split(train, y, test_size=dev_size, random_state=i)
    watchlist = [(xgb.DMatrix(x1, y1), 'train'), (xgb.DMatrix(x2, y2), 'valid')]
    model = xgb.train(params, xgb.DMatrix(x1, y1), num_rounds,  watchlist, verbose_eval=1, early_stopping_rounds=early_stopping_rounds)
...
preds /= denom # I guess to take the average from the cross-val folds..
